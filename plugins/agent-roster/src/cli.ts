...
function helpText(): string {
  return [
    ...
    "  bb roster create --name <name> --role <role> --prompt <text> [--avatar <emoji>] [--model <id>] [--tools a,b] [--project <id>]",
    "  bb roster update <agent-id> [--name] [--role] [--prompt] [--avatar] [--model] [--tools a,b] [--project <id>]",
      ...
      try {
        ...
        if (command === "create") {
          ...
          const agent = await store.registerAgent(projectId, {
            ...
            avatar: flagString(parsed, "avatar") ?? "🤖",
