...
export function graphifyOutDirectory(source: ProjectSource): string {
  return `${source.rootPath}/graphify-out`;
}

export function graphJsonPath(source: ProjectSource): string {
  return `${graphifyOutDirectory(source)}/graph.json`;
}
